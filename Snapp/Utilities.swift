...
func traverseJson(json: Any, path: String) -> Any? {
    let keys = path.split(separator: ".").map(String.init)
    
    var position: Any? = json
    for key in keys {
        let pattern = "\\[\\d+\\]"
        let predicate = NSPredicate(format: "self MATCHES %@", pattern)
        if predicate.evaluate(with: key) {
            if let jsonArray = position as? [Any] {
                let index = Int(key.dropFirst().dropLast())!
                position = jsonArray[index]
            }
        } else {
            if let jsonDictionary = position as? [String: Any] {
                position = jsonDictionary[key]
            }
        }
    }
